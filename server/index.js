...
const stageProgress = delivery => {
  ...
  const clockStart = delivery.acceptedAt || delivery.assignedAt || delivery.createdAt;
  const minutes = Math.max(0, (Date.now() - new Date(clockStart).getTime()) / 60000);
  ...
  if (delivery.driverId || stage.status === 'LOOKING_FOR_RIDER') {
    if (stage.status !== 'LOOKING_FOR_RIDER') delivery.status = stage.status;
...
const advanceDelivery = (delivery, actor) => {
  ...
  if (next.status === 'RIDER_ACCEPTED') delivery.acceptedAt ||= new Date().toISOString();
...
const scopedState = user => {
  ...
  if (user.role === 'BUSINESS') { const business = db.businesses.find(item => item.ownerId === user.id); const deliveries = db.deliveries.filter(item => item.businessId === business?.id); return { ...base, businesses: business ? [business] : [], drivers: db.drivers.filter(driver => deliveries.some(item => item.driverId === driver.id)), deliveries }; }
  const deliveries = db.deliveries.filter(item => item.customerId === user.id); return { ...base, drivers: db.drivers.filter(driver => deliveries.some(item => item.driverId === driver.id)), deliveries };
};
...
app.get('/api/me', (req, res) => res.json(req.user ? safe(req.user) : null));
...
app.post('/api/deliveries/:id/status', auth(), async (req, res) => { const delivery = db.deliveries.find(item => item.id === req.params.id); if (!delivery || !canAccess(req.user, delivery)) return res.status(403).json({ error: 'You do not have access to this delivery' }); if (!['ADMIN', 'RIDER'].includes(req.user.role)) return res.status(403).json({ error: 'Only the assigned rider or an administrator can advance delivery stages' }); if (req.user.role === 'RIDER' && delivery.driverId !== riderFor(req.user)?.id) return res.status(403).json({ error: 'Only the assigned rider can advance this delivery' }); if (!advanceDelivery(delivery, req.user)) return res.status(409).json({ error: 'No available rider is currently available' }); await save(); res.json(delivery); });
